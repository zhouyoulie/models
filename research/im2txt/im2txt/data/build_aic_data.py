...
def _create_vocab(captions):
  ...
  word_counts = [x for x in counter.items() if x[1] >= FLAGS.min_word_count and len(x[0].strip()) > 0]
...
def _load_and_process_metadata(captions_file, image_dir):
  ...
  id_list = [x["image_id"] for x in caption_data]
  id_to_filename = [(y, x) for (y, x) in enumerate(id_list)]
  ...
  for annotation in caption_data:
    ...
    #captions = [item_caption for item_caption in caption if len(item_caption) > 0]
    captions = [item_caption.encode("utf-8") for item_caption in caption if len(item_caption) > 0]
    #print("1"*20)
    #print(captions[0])
    #print("%r"%captions[0])
    ...
    id_to_captions[image_id].extend(captions)

  assert len(id_to_filename) == len(id_to_captions)
  assert set([item[1] for item in id_to_filename]) == set(id_to_captions.keys())
  ...
  image_metadata = []
  num_captions = 0
  for item in id_to_filename:
    #image_id is also base_filename
    base_filename = item[1] 
    image_id = item[0]
